...
import { GetDiscussions, SendDiscussionMessage, UpdateProposalStatus } from "../api/Negotiation";
...
export default function NegotiationChat({
  ...
  const [newProposal, setNewProposal] = useState<DraftProposal>({
    type: "Salary",
    proposedValue: "",
  });
  ...
  const handleProposalAction = async (
    ...
    try {
      ...
      const actionMessage = status === "Accepted"
        ? `✅ ${currentUserType} accepts this proposal.`
        : `❌ ${currentUserType} rejects this proposal.`;
    ...
    <div className="card shadow-sm border-0 mb-4">
      ...
      >
        {loading ? (
          ...
          <div className="text-center py-5">
            ...
            <p className="text-muted small mb-0">No messages yet. Start the negotiation.</p>
          </div>
        ) : (
          <>
            {messages.map((msg) => {
              const isMe = msg.senderUserType === currentUserType;
              return (
                <div
                  key={msg.id}
                ...
                >
                  ...
                  <small className="text-muted mb-1 px-1">
                    ...
                    {dayjs(msg.dateCreated).format("DD MMM, h:mm A")}
                  </small>
                  ...
                  {msg.proposals?.length > 0 && (
                    <div className="d-flex flex-column gap-2 mt-1" style={{ maxWidth: "90%" }}>
                      {msg.proposals.map((p: JobOfferProposal) => {
                        ...
                        // User can act only on other's pending proposals
                        const canAct =
                          (currentUserType !== msg.senderUserType) &&
                          isPending;
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <span className="text-black" style={{ minWidth: "80px" }}>
                                {PROPOSAL_LABELS[p.type] ?? p.type}
                              </span>
                            ...
                            {/* Action buttons for pending proposals from other user */}
                            {canAct && (
                              <div className="d-flex" style={{ borderTop: "1px solid #dee2e6" }}>
                                ...
                                >
                                  <ThumbsUp size={16} className="me-2" />Accept
                                </button>
                                <button
                                ...
                                >
                                  <ThumbsDown size={16} className="me-2" />Reject
                                </button>
                              </div>
                            )}
                          </div>
      ...
      {error && (
        <div className="px-3 pt-2">
          <div className="alert alert-danger alert-sm py-2 mb-0 small">{error}</div>
        </div>
      )}
